...
| `fortinet-fortios` | .1.3.6.1.4.1.12356 | FORTINET-FORTIGATE-MIB `fgSystemInfo` (CPU %, memory % and capacity, **active sessions**, session setup rate), `fgHwSensorEntTable` (temperature and fan), FORTINET-CORE-MIB `fnSysSerial` for identity | **Unvalidated — no hardware.** Session gauges are the firewall-specific part admitted by ADR-021. FortiOS publishes **no session ceiling**, so utilization is not answerable here, only the raw count. Memory arrives as a percentage plus a KB capacity, so used bytes are derived rather than read. One sensor table mixes temperatures, fan speeds and voltages, told apart only by the sensor's *name* — a fan at 8000 RPM charted as a temperature reads as a fire, so names are matched; voltages are dropped because no voltage metric exists in the catalogue. `fgHwSensorEntAlarmStatus` exists and is deliberately unread: its polarity is the inverse of this project's status convention and unverifiable without a unit. The sensor table is absent entirely on FortiGate-VM, which is the biggest difference between two otherwise identical-looking FortiGates. **Feeds the Flow tab over NetFlow v9 or IPFIX** (ADR-022); FortiOS also offers sFlow, which NetInv does not decode |
| `paloalto-panos` | .1.3.6.1.4.1.25461 | PAN-COMMON-MIB `panSession` (**active sessions and the platform ceiling**), `panSys` (serial, software and hardware version), **HOST-RESOURCES-MIB** for CPU (`hrProcessorLoad`, per core) and memory (`hrStorage`) | **Unvalidated — no hardware.** The only platform here whose CPU and memory come from HOST-RESOURCES rather than a vendor MIB or UCD-SNMP; the generic base reads UCD, which PAN-OS does not serve, so without this the device would report interfaces and sessions and no health at all. `hrStorage` carries RAM, swap and every mounted filesystem in one table, so the RAM row is selected by `hrStorageType` with the description as fallback — taking the wrong row reports the firewall's disk as its memory. PAN-OS *does* publish a session ceiling, so utilization is a query over count and max rather than a stored number that could drift from them. `panSessionUtilization` and the per-protocol session counts are deliberately not collected: the former is derivable, and the latter would put `netinv_firewall_session_count` in the store both with and without a `protocol` label, so any `sum()` would count every session twice. **Feeds the Flow tab over NetFlow v9** (ADR-022), the only export format PAN-OS offers |
| `ubiquiti` | .1.3.6.1.4.1.41112 (+ .10002 airOS), **plus sysDescr match** | inherits the generic UCD-SNMP/LM-SENSORS health set | **Validated against real UDM-Pro hardware**: UniFi OS consoles run stock net-snmp and report sysObjectID `.1.3.6.1.4.1.8072.3.2.10`, so prefix matching alone misses them — hence the sysDescr fallback. They expose UCD-SNMP + LM-SENSORS but *not* the UniFi MIB, HOST-RESOURCES, or LLDP. UniFi Controller REST connector is roadmap (ADR/C3). **These gateways do export flow**, contrary to what this table said until 2026-08-12: UniFi Network carries a Traffic Logging → NetFlow (IPFIX) panel offering v5/v9/IPFIX, and a pilot gateway's v5 export decodes end to end (doc 34 §4.2). It is selected per *network* rather than per interface, and ships with sampling on. **Site-to-site SD-WAN tunnels** surface in IF-MIB as WireGuard interfaces named `wgstsNNNN`, and the tunnel number is the *same on both endpoints* — that pairs them into weathermap links without any controller API. On gateways that populate it, `ifAlias` also names the peer outright (`Magic S2S VPN: FN Gateway(Default) - AL Gateway(Default)`), though it is empty on some units, so match on the `wgsts` number and treat the alias as a bonus. These interfaces report `ifSpeed 0`, so weathermap links over them need a capacity set by hand or they never colour |
...
| `huawei`, `juniper`, `zte`, `fortinet`, `paloalto` | **no** | no hardware, no simulator profile — this is the gap a [hardware validation report](../.github/ISSUE_TEMPLATE/hardware_validation.yml) closes |
