...
**E2E (Playwright, against `docker compose` stack + snmpsim):** ~30 journeys — login/lockout/refresh-expiry; onboard device → watch first metrics arrive; inventory search/filter/export CSV+XLSX; ack alert from dashboard → deep-link lands on the explaining graph (PRD G3); weathermap: create map, add nodes, bind link, see live coloring (mocked utilization step-changes), publish, viewer sees update; RBAC: each role sees/loses the right nav+actions; settings/SMTP test-send (MailHog); audit trail reflects the session's actions.
