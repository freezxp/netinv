...
`GET /interfaces` searches every device's ports by **alias, description or interface name**, case-insensitive substring, returning `{data, total}` with the owning device on each row. It is deliberately not device-scoped: alias is the field operators curate — circuit ids, customer names, the far end of a link — and reading it one device at a time meant "which port is the London circuit on" could only be answered by someone who already knew. All three fields are matched together because whoever labelled the port may have used any of them. Removed interfaces and retired devices are excluded: a search turns up ports you can act on, not history. Utilization is not returned — the caller derives it from the counters, since `speed_bps` is on the row and an unknown speed has to read as unknown rather than as a percentage of nothing. `up=1` excludes interfaces whose last synced oper status is not up — filtered in the database so `total` and paging follow it, and matching the value the list displays. Unknown oper status is *not* treated as up: an interface nobody has ever recorded a status for is not evidence of a working port. There is deliberately no server-side "idle" filter, because traffic lives in the metrics store and there is no column to filter on; the UI applies that to the rows it already has and says how many it removed. `sort` takes one of `device|name|customer|alias|descr|speed|state` with `dir=asc|desc`, from a whitelist — the value is concatenated into an ORDER BY, and taking that from a URL parameter is how a search box becomes an injection; an unrecognised key falls back to the default order rather than erroring. NULLs sort last in **both** directions, because an interface with no alias or no speed is missing information and missing information belongs at the end of a list read top-down. Every ordering ends in a stable tiebreak, without which two rows sharing a value can swap between pages and be seen twice or missed while paging. Sorting by traffic or utilisation is not offered here for the same reason as the idle filter.
...
| POST | `/devices/{id}/merge` (fold a duplicate into this device) | `devices:write` | 200, 409 (already retired) |
...
`POST /devices/{id}/merge` folds a duplicate into this device: its management address and any alternates are recorded here, its tags are unioned in, its polling schedules are disabled, and it is **retired — not purged**. The response reports `metrics_moved: false` and `history_moved: false`, and that is the point rather than a limitation to hide. Every series is keyed by `device_id` in VictoriaMetrics and the duplicate's asset history and topology links point at its own interface rows, so "moving" them would mean rewriting the metrics store and relabelling a year of data that two separate records genuinely did collect separately. What was collected under the duplicate stays readable under it; everything from now on is collected under the survivor. Purging instead of retiring would destroy exactly the data the merge is meant to preserve.
