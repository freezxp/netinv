...
| `sites.active` | fanout | ephemeral per-poller queues (exclusive, auto-delete, server-named) | scheduler → pollers: the set of sites being dispatched to. A poller holds only AMQP — no database, no API credentials — so this is the only channel by which it can learn that a site exists. One configured with `NETINV_SITE_ID=*` starts and stops per-site consumers to match, which is what stops a site created after the poller started from being silently unpolled. Published on a timer as a full list rather than as deltas: a poller that missed a message, or started late, must converge without anyone creating a site to trigger it. Exclusive queues because every poller needs the whole list — a shared durable queue would round-robin the announcements and leave each with half. |
