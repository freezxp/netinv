...
- **Viewer:** full-bleed canvas; pan/zoom (wheel/pinch), fit/100% controls; legend (utilization scale + node states) collapsible; link hover → tooltip (the heavier direction the band shows, plus the quieter reverse figure, %, errors, speed) with 1h mini-sparkline; link click → interface detail; node click → device peek drawer (status, top alerts, CPU/mem) with "open device". Auto-refresh ≤30 s (FR-MAP-05); presence-aware (doc 05 §7). Fullscreen/wall mode.
