...
- **Graph time range (FR-DASH-09, partial):** one shared selector — `1h · 6h · 24h · 7d`, default 24h — applies to every time-series chart in the portal and persists in `localStorage` across pages and reloads. An operator holding one question ("what did last week look like?") should not have to re-answer it on each page, and previously could not: the dashboard was fixed at 24h, a device's traffic chart at 6h and its health charts at 24h, with nothing on screen saying which. Every chart card now names the window it is showing. The control appears with the charts, not at the top of the page, because the dashboard's status strip and the availability tiles are server-computed over fixed windows and are deliberately unaffected; on device detail it is hidden on History and Alerts, whose tables would otherwise appear to be filtered by it. The weathermap viewer carries one too: its link hover graph follows the same range, but that card is a transient overlay with pointer events disabled, so without a control on the page chrome the map would be the one place showing a window you cannot change. **Resolution and rate windows scale with the range** (`src/api/timerange.ts`): step is chosen to land each range near 150–350 points, and `rate()` lookbacks derive from the step rather than a fixed `[5m]` — at a 30-minute step a 5-minute lookback measures a sixth of each bucket and reports it as the whole, which renders a week of traffic as noise with wrong peaks. Consequence worth knowing: wide ranges show sustained averages, so a 7d peak reads lower than the same traffic at 1h. That is correct for capacity work; percentile views are the refinement, not a fixed narrow window. **Not yet implemented from FR-DASH-09:** the `30d` preset and a custom range picker. 30d is deliberately deferred until doc 04's downsampling tiers exist — at raw resolution a month of data is a large query per chart per viewer, which is exactly what FR-DASH-01..08 forbids. A custom range also needs URL state so a range can be shared in a link, which the preset-only version does not.
