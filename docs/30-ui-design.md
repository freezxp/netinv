...
- **Responsive** (NFR-60): breakpoints ≥1280 full grid · 768–1280 dashboard collapses to 2-col, tables shed secondary columns · <768 read-mostly (status row, alert list, ack actions; weathermap view-only pan/zoom; editor desktop-only). NOC-wall mode: `/wall` route — dashboard variant with no chrome, larger type, auto-cycling optional.
...
- **Health:** full graph set per family — CPU per processor, memory pools, temperature sensors, fan RPM, PSU state timeline, optics per port; component inventory table (kind, name, model/serial, state).
