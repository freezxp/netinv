...
- **FR-ALR-07** Ships with a default rule pack: device down (no ICMP **and** no successful SNMP poll) critical, with ping-only loss split off as its own warning; interface down (oper≠admin) warning; utilization >80%/15m warning, >90% critical; CPU >85%/10m; memory >90%; temperature above vendor threshold; PSU/fan failed; device rebooted (sysUpTime reset) info; poll auth-failure warning; **flow exporter stopped exporting** warning, **no flow received from any exporter** warning, and **availability probe cannot run** warning.
  - The two flow rules exist because collection can stop with nothing saying so, and did: on the pilot two of three exporters stopped within a minute of each other and went unnoticed for **42 hours**, because the third kept exporting and every fleet-wide flow query stayed non-empty. **Partial loss is therefore the case the pack must cover**, not just total loss — a rule asking only "is any flow arriving" stays green throughout. `ar_flow_exporter_stale` is the primary rule: it subtracts exporters seen in the last 20 min from those seen in the last 7 d, so it reports one alert per stale exporter carrying the `exporter` label, covers total loss as well as partial (every exporter is stale when all stop), and self-gates — a deployment that never received flow has nothing in the long window and stays silent, which matters because flow is optional (ADR-020). `ar_flow_absent` extends that past the 7 d window and **must stay gated on `count(last_over_time(…[30d]))`**: bare `absent_over_time` returns 1 for a metric that never existed, so ungated it fires on every install without flow — the same mistake FR-ALR-08 rejects for interfaces never in service. Both windows are ceilings past which the alert resolves while the fault persists; doc 34 §5.1 records the inventory-derived replacement that removes them.
  - `ar_device_down` requires **both** signals to be silent, and `ar_icmp_down` covers the case where only ping is. Until 2026-08-18 device-down was ICMP alone, so a device answering SNMP perfectly — syncing inventory, advancing counters, filling graphs — raised a critical because echo replies stopped, which happens for reasons unrelated to the device being down: a management ACL permitting udp/161 and omitting ICMP, control-plane ICMP policing, or the poller's own unprivileged-ping path breaking inside a container. Waking someone for a device that is demonstrably answering is how an alert stops being believed. The device-down expression is written as `unless` against *SNMP-is-working* rather than `and` against *SNMP-is-failing*, and that is load-bearing: a device with no SNMP agent at all — an ICMP-only polling profile, which is what a mesh-joined AP runs on — has no `netinv_poll_success` series, so with `and` its side of the join would be empty and the one device whose only signal is ping could never alert on losing it. The SNMP window is 10 minutes against ICMP's 3, because the family cadences differ (traffic 60s, health 300s) and a window that cannot span the slowest scheduled family would read a healthy device as having no SNMP and escalate to critical.
  - `ar_util_warn` and `ar_util_crit` read `netinv_if_utilization_percent` from Sprint 9 until 2026-08-18, and **nothing has ever written that metric** — the collector publishes the octet counters and `netinv_if_speed_bps`, and utilization is derived at query time by every reader. Both rules therefore evaluated an empty selector and stayed silent for the life of the product; a rule that cannot fire is indistinguishable from a network that is behaving. Migration 0015 computes utilization in the expression instead, taking the busier direction via the `label_set`/`or` shape (a bare `or` collapses in and out and drops one) and dividing by `(netinv_if_speed_bps > 0)` — the filter is load-bearing, since an unknown or zero speed divides to +Inf and would fire permanently on exactly the ports where ifSpeed is missing.
  - `ar_icmp_probe_error` exists because device-down alerting could stop working silently. Its rule is `max_over_time(netinv_icmp_up[3m]) == 0`, which needs the series to exist — and when the probe *itself* could not run, the poller wrote no sample at all rather than a zero. An empty lookbehind yields no series, so the rule matched nothing and went quiet at precisely the moment availability stopped being measured; the unprivileged-ping case inside an LXC, where `net.ipv4.ping_group_range` excludes the poller's gid, does exactly this while SNMP keeps working. The poller now writes `netinv_icmp_probe_error` — 1 when the probe could not run, 0 on every success, so the series is continuous and can be matched with `== 1` without also catching devices never probed. It deliberately does **not** report the device down instead: the poller does not know whether it is, and paging someone to healthy equipment because we failed to ask is the worse error. Severity is warning rather than critical for the same reason — nothing in the network is known to be broken, but the thing that would have told you is.
  - Both packs seed with `ON CONFLICT DO NOTHING` and no conflict target. `alert_rules` carries `UNIQUE (tenant_id, name)`, so on a deployment where an operator already created a rule with a shipped rule's name, a bare INSERT aborts the migration — and an aborted migration exits the api at startup, on every start, stranding the deployment several versions behind with only a crashloop to explain it. Naming the target as `(id)` looks like a fix and is not: the constraint that bites is the name.
...
- **FR-MAP-03** Links bind to one or two directed interface endpoints. **One bound end is enough**: rates are read from the A endpoint when it is bound and from B otherwise, mirroring in/out so both stay relative to the link's A side (what B receives is what A sent). A link commonly has only one pollable end — a mesh AP running no SNMP agent, an ISP cloud, any site/cloud/label node — and one interface describes the whole wire either way; reading only A left such links blank for no better reason than which direction they happened to be drawn in. Rendered as **one band per link, running in whichever direction carries more traffic**, coloured by live utilization on the classic scale (0–1-10-25-40-55-70-85-100%). This replaced Cacti's split line — each half coloured by the traffic flowing toward the node it touched — which was faithful but busy: every link carried two colours, two arrowheads and two numbers, so the pilot's thirteen-link mesh presented fifty-two things to read and the eye had to compare halves before learning anything. A link's interesting property is usually its heavier direction, so that is what the band's colour and dash animation follow. **Both directions still carry an arrowhead and a rate**, because a speed without a direction is half an answer — but the reverse pair is drawn smaller and dimmer, so the map reads at a glance and rewards a closer look rather than demanding one. Comparing bytes per second is equivalent to comparing utilization, because a link has one capacity shared by both directions, and ties fall to the A→B direction so an idle link draws consistently rather than flickering on rounding noise. Direction must be readable without interaction and without a legend: the band carries an arrowhead placed partway along it — clear of the node boxes, which otherwise swallow an arrow drawn at the line's end — and dashes crawl along it in the direction of travel whenever it carries traffic, so the busy direction is apparent on a diagonal as much as a horizontal. Motion is decorative only (arrowheads and labels carry the same information) and is dropped under `prefers-reduced-motion`. Rate labels are haloed in the page background so they stay legible over any utilization color, and sit beside rather than across steep links, where a centered label would other­wise cover the arrowhead it belongs to. **Each rate names its destination** — `→YY 64.3 Kbps · 17%` — and sits beside the arrowhead it describes. Position alone was not enough to pair a number with a direction: the bright figure sat at 30% of the line while its arrow was at 58%, so nothing tied them together and readers had to guess. Naming the far end settles it from the text, which is what survives crossing links and steep angles. Node names are shortened to their first word (`YY Gateway` → `YY`); the tooltip keeps the full names. The two directions sit either side of centre, each next to the arrowhead it describes, and on **opposite sides of the line** — offsetting both the same way stacked "7.2 Kbps" and "1 bps" into an unreadable "0%1 bps" on short links. Neither sits at the midpoint, because two links that cross usually cross near their middles and the pilot mesh has exactly that pair. Labels belonging to *different* crossing links can still touch on a dense map; nodes can be dragged apart, and genuine collision avoidance — measuring text and nudging labels along their lines — is unbuilt.
