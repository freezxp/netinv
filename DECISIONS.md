...
- Live pilot network detail (management addresses) was removed from CLAUDE.md before publication, and the two RFC1918 addresses that appeared in commit message bodies were scrubbed by rewriting those two messages. The rewrite covers the 32 newest commits and changes their hashes; anyone holding a clone from before publication must re-clone rather than pull. The commits cited elsewhere for the historical gitleaks finding — e079092 and 77c5de9, referenced in `.gitleaks.toml` and doc 25 §5.1 — are ancestors of the rewrite point and keep their hashes.
