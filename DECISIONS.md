...
- Live pilot network detail (management addresses) was removed from CLAUDE.md before publication. Two RFC1918 addresses remain in commit message bodies (a `10.0.31.8` and a `10.0.30.0/24`, in the commits adding sysName-led inventory and subnet discovery). Rewriting them was prepared and then declined: both are private-range addresses that are unroutable from outside the network they describe, they carry no credential, and the rewrite would have changed the hashes of the 32 newest commits — breaking every existing clone and invalidating hashes cited elsewhere in these docs. The exposure is that a reader learns two subnets of a network they cannot reach. That was judged acceptable; the same is **not** true of anything routable, any hostname identifying the owner's organisation, or any serial number, and those must never be committed.
