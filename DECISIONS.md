...
**Consequences:**
- Cardinality is bounded by construction: N talkers × interfaces, not host-pairs. N is configurable (`NETINV_FLOW_TOPN`) because the right depth depends on fleet size; the default rose from 10 to 25 on 2026-08-12 after real hardware showed every interface pinned at the cap in every interval, with doc 34 §1 carrying the arithmetic that tells a large fleet to lower it. The top-N cut happens in the collector, before anything is stored, and is therefore not recoverable later — a conversation outside the top N for an interval did not merely go unqueried, it was never written.
...
**Decision:** Decode NetFlow v9, including options templates, and apply the sampling interval an exporter announces through them. IPFIX and sFlow remain undecoded. **Amended the same day:** IPFIX is decoded too. Once the template machinery existed, IPFIX was that machinery plus a 16-byte header carrying a byte length rather than a record count, Set IDs 2 and 3, a scope *count* where v9 has a scope *byte length*, enterprise-specific element IDs, and variable-length fields — differences worth a day, not an ADR of their own. sFlow remains undecoded and needs a decoder that shares none of this.
